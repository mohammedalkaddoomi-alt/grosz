...
import type { User, Wallet, Transaction, WageSettings, WorkCalculation, Subscription } from '../types';
...
const createSupabaseStore = () => create<AppState>((set, get) => ({
    ...
    loadData: async () => {
        ...
        try {
            const [walletsData, categories, wageSettings, subscriptions] = await Promise.all([
    ...
    inviteToWallet: async (walletId, email) => {
        await db.addWalletMember(walletId, email);
        await get().loadData();
    },
    ...
    loadGoals: async () => {
        const user = get().user;
        if (!user) return;
        const goalsData = await db.getGoals(user.id);
    ...
    // Security ActionsImplementation
    lockApp: () => {
